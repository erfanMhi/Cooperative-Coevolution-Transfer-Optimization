...
class TransferProcess(mp.Process):
  def __init__(self, dims, problem, sample_size, 
              src_models, shared_queue, shared_array,
              t_lock, list_lock, transfer_interval=2):
  ...
  def _transfer_ea(self):
    ...
    while True:
      ...
      #                           np.concatenate((fitness, cfitness)), self.psize)
...
def main_multi(args):
  ...
  for i in range(reps):
    ...
    tr_process = TransferProcess(dims, target_problem, sample_size, src_models,
